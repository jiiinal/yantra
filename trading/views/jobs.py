...
def getSymbols(request):
    if request.method != 'GET':
        ...
        # symbols is already a values() queryset, so list() works directly
        return Response(list(symbols), status=status.HTTP_200_OK)
...
def testConnection(request):
    if request.method != 'POST':
        ...
        try:
            ...
            if quote:
                logger.debug(f'Test connection success {request.user}')
            ...
            logger.debug(f'Test connection error {request.user} {e.args[0]}')
            traceback.print_exc()
            return Response({'status': 'error', 'message': str(e)}, status=status.HTTP_400_BAD_REQUEST)
